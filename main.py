...
# If QUESTIONS has less than 25 entries due to formatting, ensure length is 25:
# (Above we gave 25 entries.)
...
class SplashScreen(BaseScreen):
    def on_enter(self):
        # auto go to welcome after 2 seconds
...
class QuestionScreen(BaseScreen):
    ...
    scores = ListProperty([0, 0, 0, 0, 0])  # parallel to LOVE_LANGUAGES
    total_questions = NumericProperty(0)

    # dynamic list yang akan dipakai sesuai purpose
    ...
    def set_question_list(self, questions, purpose):
        ...
        # Ambil 20 pertanyaan acak dari list (tanpa merusak urutan aslinya)
        if len(questions) > 20:
            self.current_questions = random.sample(questions, 20)
        else:
            self.current_questions = questions[:]   # fallback kalau isinya <20
    ...
    def on_enter(self):
        ...
        # Jika belum ada question set, jangan load (atau fallback ke self)
        if not self.current_questions:
            # fallback: gunakan QUESTIONS_SELF jika ada
    ...
    def load_question(self):
        ...
        if self.q_index < self.total_questions:
            ...
            # Pairing opsi + index asli
            option_pairs = [(text, idx) for idx, text in enumerate(options)]

            # Shuffle
            random.shuffle(option_pairs)

            # Masukkan ke tombol (pastikan KV punya opt0..opt4)
        ...
        if self.q_index in self.answers:
            ...
            for i in range(5):
                ...
                if btn and btn.lang_index == selected_lang:
                    btn.background_color = (0.85, 0.25, 0.45, 1)  # warna aktif

        
    def choose(self, lang_index):
        if self.q_index not in self.answers:   # hanya boleh isi sekali
    ...
    def prev_question(self):
        ...
        if self.q_index in self.answers:
            ...
            # Kurangi skor & hapus jawaban sebelumnya
...
class BarChart(Widget):
    # expects percentages as list of 5 numbers (0..100)
    ...
    def draw_chart(self):
        ...
        with self.canvas:
            # background (light)
            Color(rgba=(1, 1, 1, 0))  # transparent background - keep parent bg
            # compute bar sizes
            w = self.width
            h = self.height
...
class ResultScreen(BaseScreen):
    ...
    def on_enter(self):
        ...
        # update PieChart (id di KV adalah 'pie_chart')
...
class ThankYouScreen(BaseScreen):
    ...
    def download_result(self):
        ...
        # Ambil root result yang mau di screenshot
        ...
        # Pastikan layout selesai dirender
        root_widget.canvas.ask_update()

        # Screenshot setelah layout stabil
